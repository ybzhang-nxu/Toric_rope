...
ROOT = Path("results")
OUT = ROOT / "v7_stl10_trainradial_component_mechanism_summary"
...
MODEL_SPECS = [
    {
        ...
        "path": ROOT
        / "v7_component_eval_stl10_mixed_trainradial_r4_seed426_layer_head_l0123_allheads_10k"
    ...
    },
    {
        ...
        "path": ROOT
        / "v7_component_eval_stl10_mixed_trainradial_r4_seed526_layer_head_l0123_allheads_10k"
        / "component_interventions.csv",
    },
    {
        "seed": 626,
        ...
        "path": ROOT
        / "v7_component_eval_stl10_mixed_trainradial_r4_seed626_layer_head_l0123_allheads_10k"
        / "component_interventions.csv",
    },
]
...
def key_metrics(spec: dict[str, object], rows: list[dict[str, str]]) -> dict[str, object]:
    ...
    pool_l45_keep = find_row(rows, "head_subset_keep", subset_param([4, 5]))
    pool_l45_ablate = find_row(rows, "head_subset_ablate", subset_param([4, 5]))
    pool_l012345_keep = find_row(rows, "head_subset_keep", subset_param([0, 1, 2, 3, 4, 5]))
    pool_l012345_ablate = find_row(rows, "head_subset_ablate", subset_param([0, 1, 2, 3, 4, 5]))
...
def write_report(seed_rows: list[dict[str, object]], family: dict[str, object]) -> None:
    family_rows = [
        {
            ...
            "l45_ablate": f"{float(family['l45_ablate_mean']):.4f} +/- {float(family['l45_ablate_std']):.4f}",
    ...
    text = "\n".join(
        [
            "# STL10 Train-Radial R4 Component Mechanism Summary",
            "",
            "Inputs: STL10 mixed residual-DCT train-radial r=4 seeds 426,526,626.",
            "",
            "## Family Key Metrics",
            "",
            table(
            ...
            "",
            (
                "- L0-L3 all-head keep is nearly sufficient: "
                f"{float(family['l0123_keep_mean']):.4f} +/- {float(family['l0123_keep_std']):.4f}, "
                f"near full {float(family['full_mean']):.4f} +/- {float(family['full_std']):.4f}."
            ),
            (
                "- L0-L3 all-head ablate is strongly necessary but leaves residual compensation: "
            ...
            (
                "- L4-L5 alone is weak but nonzero: "
                f"{float(family['l45_keep_mean']):.4f} +/- {float(family['l45_keep_std']):.4f}; "
                "ablate L4-L5 keeps most of the full score, so it acts more like late compensation than the main mechanism."
            ),
            (
                "- Ablating all local heads L0-L5 nearly exhausts the signal: "
                f"{float(family['l012345_ablate_mean']):.4f} +/- {float(family['l012345_ablate_std']):.4f}, "
                f"with zero at {float(family['zero_mean']):.4f} +/- {float(family['zero_std']):.4f}."
            ),
            "",
        ]
    )
